    ...
    def _joy_callback(self, msg: Joy):
        ...
        try:
            ...
            self.get_logger().error(
                f"IndexError when updating controller states: {e}. "
                f"The received Joy message has {len(msg.axes)} axes and {len(msg.buttons)} buttons, "
                f"but the configured mapping expects indices that are out of bounds. "
                f"Please ensure you have specified the correct joystick device path (e.g., using 'joy_dev:=/dev/input/js1')."
            )
            return
